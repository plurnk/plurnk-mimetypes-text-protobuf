...
export default abstract class Protobuf3ParserBase extends Parser {
    ...
    DoMessageNameDef_(): void {
        ...
        const identifier = (tctx.parent as MessageDefContext).messageName().ident();
    ...
    DoEnumNameDef_(): void {
        ...
        const identifier = (tctx.parent as EnumDefContext).enumName().ident();
        const name = identifier.getText();
    ...
    DoServiceNameDef_(): void {
        ...
        const identifier = (tctx.parent as ServiceDefContext).serviceName().ident();
        const name = identifier.getText();
    ...
    DoImportStatement_(): void {
        try {
            ...
            const current_file = this.tokenStream.tokenSource.sourceName;
            if (this.debug) {
                console.error("current file = " + current_file);
                console.error("imported file = " + import_file_name);
            }

            const save = process.cwd().replace(/\\/g, '/');
